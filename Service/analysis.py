...
def is_cyclic(expr, search, hist=set()):
    ret = False
    if search in hist:
        return True

    # get sub-prerequisites for every course in the prerequisites
    for sub_course_symbol in expr.atoms():
        hist.add(sub_course_symbol)
        if isinstance(sub_course_symbol, Symbol):
            if search is sub_course_symbol:
                return True
            else:
                if sub_course_symbol.name in prereqs_dict:
                    ret = ret or is_it_cyclic(
                        prereqs_dict[sub_course_symbol.name], sub_course_symbol)
    return ret
...
def get_all_prereqs(prerequisites, course=None, ret=set()):
    ...
    for sub_course_symbol in prerequisites.atoms():
        if isinstance(sub_course_symbol, Symbol) and sub_course_symbol.name in prereqs_dict:
            get_all_prereqs(
                prereqs_dict[sub_course_symbol.name], sub_course_symbol, ret)
    return ret
...
def prereq_implies_prereq(a, b):
    ...
    res = satisfiable_with_givens(
        a & ~b, get_all_prereqs(a) | get_all_prereqs(b))
...
def Analyze(term: str):
    ...
    cyclics = [name for (name, prereqs) in prereqs_dict.items()
               if is_cyclic(prereqs, courses_dict[name])]
    if len(cyclics):
        raise CycleError(cyclics)

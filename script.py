...
class CoursePrerequisites:
  ...
  def __init__(self, filename: str) -> None:
    self.Prerequisites = {}
    link_list = self.getCoursesLink(filename)

    for url in link_list:
        self.getCoursePrerequisites(url)

    self.savePrerequisites()
  ...
  # @completed
  def getCoursesLink(self, file_name: str) -> list:
  ...
  # @return, None
  # @completed
  def savePrerequisites(self):
      with open('CoursePrerequisites.json', 'w') as fp:
          json.dump(self.Prerequisites, fp)



if __name__ == "__main__":
  CoursePrerequisites('CourseCatalog.html')
